...
for it = 1:4
    ...
    eval(text);
    for itS = 3:8
        text = strcat('signalC.', testCase{it}, '.XData(itS-2, :) = ', testCase{it},'.hgS_070000.children(1).children(itS).properties.XData*(0.9*soundVelocity/diameter);');
        eval(text);
        text = strcat('signalC.', testCase{it}, '.YData(itS-2, :) = ', testCase{it},'.hgS_070000.children(1).children(itS).properties.YData;');
        eval(text);
    end
end
close all
